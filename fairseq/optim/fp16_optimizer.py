...
class _FP16OptimizerMixin(object):
    ...
    def build_fp32_params(cls, args, params, flatten=True):
        ...
        if flatten:
            ...
            total_param_size = sum(p['params'][0].data.numel() for p in params)
            # total_param_size = 1
            ...
            for device in devices:
                ...
                fp32_params[device] = (
                    ...
                    device_params[0]['params'][0].new(0).float().new(device_param_size)
                )
                offset = 0
                for p in device_params:
                    # numel = p.data.numel()
                    numel = p['params'][0].data.numel()
                    # fp32_params[device][offset : offset + numel].copy_(p.data.view(-1))
                    fp32_params[device][offset : offset + numel].copy_(p['params'][0].data.view(-1))
                    offset += numel
